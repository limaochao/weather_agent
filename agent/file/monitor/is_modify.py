...
LastEditTime: 2021-01-09 11:00:00
'''

import time
from agent.common import _global_dict
...
def is_modified_in_time(data_update_interval, service_name):
    """单位时间内是否有文件更新"""
    last_modify = _global_dict.get_value("modified" + service_name)
    if last_modify is None:
        _global_dict.set_value("modified" + service_name, time.time())
...
def is_modified(service_name):
    """预警是否更新"""
    last_modify = _global_dict.get_value("modified" + service_name)
    before_last_modify = _global_dict.get_value(
        "modified_before" + service_name)
    now_time = time.time()
    if last_modify is None:
        _global_dict.set_value("modified" + service_name, now_time)
        last_modify = now_time
    if before_last_modify is None:
        _global_dict.set_value(
            "modified_before" + service_name,
            _global_dict.get_value("modified" + service_name)
        )
