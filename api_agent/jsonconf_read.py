...
class HttpConf:
    ...
    '''
    返回url所需信息（数组），依次是source数据源（url地址），name，attr（含id和pass的字典），response_code
    '''

    def get_url_info(self):
        url_info = [self.http_dict['source'], self.http_dict['name'], self.http_dict['attr'],
    ...
    def get_tag(self):
        return 'department=' + self.http_dict['department'] + ',branch=' + self.http_dict['branch'] \
               ...
               + ',is_begin=' + self.http_dict['is_begin'] + ',is_finish=' + self.http_dict['is_finish'] \
               + ',leader=' + self.http_dict['leader'] \
               + ',project=' + self.http_dict['project'] + ',deputy=' + self.http_dict['deputy'] \
               + ',source=' + self.http_dict['source'] + ',name=' + self.http_dict['name']
...
class ServerConf:
    def __init__(self, input_dict):
        ...
        if isinstance(self.conf_dict, dict):
            self.http_count = len(self.conf_dict)
        else:
            raise TypeError("unexpected type of Server dict")
